...
def cleanup_conf_models():
    ...
    logger.info(
        "POST /confidentiality/cleanup from %s | params={dry_run=%s, keep_best_n=%s, max_age_days=%s, min_acc=%.2f} | raw=%s",
...
@conf_bp.route("/metrics/latest", methods=["GET"])
def get_latest_metrics():
    ...
    try:
        ...
        return jsonify({
            ...
            "image_base_url": f"/confidentiality/metrics/{session_id}"
        }), 200
...
@conf_bp.route("/metrics/sessions", methods=["GET"])
        ...
        for session_path in sessions:
            ...
            if summary_file.exists():
                ...
                sessions_list.append({
                    ...
                    "best_f1_score": summary_data.get("best_model", {}).get("metrics", {}).get("f1_macro"),
                    "total_models": summary_data.get("dataset_info", {}).get("total_models_trained"),
                    "url": f"/confidentiality/metrics/{session_id}"
                })
            else:
                sessions_list.append({
                    "session_id": session_id,
...
@conf_bp.route("/metrics/<session_id>", methods=["GET"])
...
@conf_bp.route("/metrics/<session_id>/<filename>", methods=["GET"])
...
@conf_bp.route("/metrics/compare", methods=["POST"])
