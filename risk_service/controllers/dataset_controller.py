...
# Endpoints (igual ao confidentiality_controller.py, adaptado para risk)
    ...
    try:
        ...
        for item in DATA_DIR.iterdir():
            if item.is_file():
                ...
                modified = max((f.stat().st_mtime for f in item.iterdir() if f.is_file()), default=item.stat().st_mtime)
                datasets.append({
                    "name": item.name,
                    "type": "directory",
                    "path": str(item.relative_to(DATA_DIR)),
        ...
        logger.info("GET /datasets from %s | found=%d | elapsed_ms=%d", client_ip, len(datasets), int((time.time() - t0) * 1000))
        return jsonify({"total": len(datasets), "datasets": sorted(datasets, key=lambda x: x.get("modified_at", 0), reverse=True)}), 200
